...
def update_user(user_id: str):
    ...
    return _ok({"message": "Budget updated.", "monthly_budget": budget})
...
def demo_load():
    ...
    all_txns = db.get_transactions(user_id)
    analysis, forecast, profile, alerts = _run_full_pipeline(user_id, user, all_txns)
...
def suggest():
    ...
    # Pass full profile context so Gemini gets income, savings, behaviour
    result = generate_suggestions(analysis, forecast, profile)
...
def get_alerts():
    ...
    _require_user(user_id)
    alerts = db.get_alerts(user_id, unread_only=unread_only, limit=limit)
    counts = {"critical": 0, "warning": 0, "info": 0}
    for a in alerts:
        counts[a["severity"]] = counts.get(a["severity"], 0) + 1

    return _ok({"user_id": user_id, "total": len(alerts),
                "severity_counts": counts, "alerts": alerts})
