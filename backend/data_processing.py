...
def parse_transactions(raw_data: list[dict]) -> list[dict]:
    ...
    Credits stored as negative amounts (income),
    debits as positive amounts (expense).
    """
    ...
    for raw in raw_data:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid or missing 'date' in record {raw}: {exc}"
            )
        ...
        try:
            ...
            raise ValueError(
                f"Invalid or missing 'amount' in record {raw}: {exc}"
            )
        ...
        if txn_type not in ("debit", "credit"):
            raise ValueError(
                f"Invalid 'type' in record {raw}. "
                f"Allowed values: debit or credit."
            )
